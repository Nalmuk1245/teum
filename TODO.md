...
## 🟠 상장따리 (신규 상장 캐치)
- [x] **업비트/빗썸 신규 상장 감지** — market/all 3초 폴링 diff(실거래 개시 순간), 해외 상장 여부(김프 가능성) 판정, 텔레그램 즉시 알림, 보드 최상단(+1000)·신규상장 배지, 관제 탭 최근상장 카드
- [ ] 공지 사전감지(api-manager) — 이 박스는 Cloudflare 차단, 로컬 KR PC에서 재시도
- [ ] 신규 상장 원클릭 실행 프리셋(해외 매수 대기)
