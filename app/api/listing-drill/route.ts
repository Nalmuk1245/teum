...
export async function POST(req: Request) {
  try {
    const body = (await req.json().catch(() => ({}))) as { base?: string; openInSec?: number };
    const base = (body.base ?? "PEPE").toUpperCase();
    const openInSec = Number(body.openInSec);
    await startDrill(base, Number.isFinite(openInSec) && openInSec > 0 ? Math.min(3600, openInSec) * 1000 : undefined);
